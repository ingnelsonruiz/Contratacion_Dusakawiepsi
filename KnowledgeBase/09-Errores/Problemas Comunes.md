...
## 14. Drill-down de Top Impacto recalculaba consultas pesadas que no usaba

- **Dónde**: `abrirDrillPrestador()` en `src/components/top-impacto/top-impacto-client.tsx`, llamando a `getTopImpacto()` (`src/app/actions/top-impacto-actions.ts`) para el Nivel 2 del drill-down (prestador → códigos).
- **Síntoma reportado por el usuario (2026-07-31)**: al hacer doble clic sobre una barra de "Top 20 prestadores por valor radicado", el modal tardaba mucho en abrir, "parece que se colgara". Cita textual: *"cuando doy dble clic se demora mucho parece que se colgara pede mejora la velocidad de esa consuta"*.
- **Causa**: el diseño original (documentado en `types/top-impacto.ts`, comentario sobre Nivel 2) reutilizaba `getTopImpacto()` COMPLETO para el drill-down, fijando `ips` al prestador de la barra elegida. Pero con `ips` ya fijo a un solo prestador, 2 de las 3 consultas secuenciales pesadas que corre esa función (`obtenerPorPrestador`, `obtenerPorMunicipio` — cada una ~3-10s sobre RIPS completo) solo pueden devolver, respectivamente, ese mismo prestador en el puesto 1 y su(s) propio(s) municipio(s) — datos que el modal de Nivel 2 nunca muestra (usa únicamente `top100`/`kpis`, ver `drillNivel2.top100` en el componente). Es decir, se pagaban ~2/3 del costo total de la función en cada doble clic sin usar el resultado para nada.
- **Mitigación**: ver [[Soluciones#14. Saltar consultas no usadas en el drill-down de Top Impacto]].
